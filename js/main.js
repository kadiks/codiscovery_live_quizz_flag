...
const init = async () => {
  ...
  panels.answer.querySelector("button").addEventListener("click", () => {
    if (questionNumber <= questionTotal) {
      ...
      panels.end.querySelector(
        "p"
      ).innerHTML = `Your score is: ${goodAnswers} / ${questionTotal}`;
      switchPanel("end");
    }
  });
...
const checkAnswer = (userAnswer) => {
  ...
  if (userAnswer === question.answer) {
    ...
    panels.answer.querySelector(
      "p"
    ).innerHTML = `The answer was: ${question.answer}`;
  }
  questionNumber++;
  switchPanel("answer");
};
